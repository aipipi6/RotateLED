...
//data:要写入的数据
//返回值:读到的数据
u8 SD_ReadWriteByte(u8 data)
{
	return SPI_ReadWriteByte(SD_SPIx, data);
}	  
//SD卡初始化的时候,需要低速
void SD_SpeedLow(void)
{
 	SPI_SetSpeed(SD_SPIx, SPI_BaudRatePrescaler_256);//设置到低速模式	
}
//SD卡正常工作的时候,可以高速了
void SD_SpeedHigh(void)
{
 	SPI_SetSpeed(SD_SPIx, SPI_BaudRatePrescaler_2);//设置到高速模式	
}
//SD硬件层初始化
void SD_HardwareInit(void)
{
	IO_OutputInit(SD_CS);  // 配置IO
